...
model_dir=attention_output/$TASK/base
if [[ $OPTIONS == *"--reverse_freezing"* ]]; then
    REVERSE=true
    model_dir=attention_output/$TASK/reverse
elif [[ $OPTIONS == *"--incremental_freezing"* ]]; then
    INCREMENTAL=true
    model_dir=attention_output/$TASK/incremental
elif [[ $OPTIONS == *"--reverse_incremental_freezing"* ]]; then
    REVERSE_INCREMENTAL=true
    model_dir=attention_output/$TASK/reverse_incremental
fi
...
do
    ...
    if [ "$REVERSE" = true ] ; then
        ...
        do
            ...
            mask_str+="bert.encoder.layer.${layer_to_freeze}.attention.output.dense "
        done
        ...
        mask_str="bert.encoder.layer.${layer}.attention.output.dense"
    fi
